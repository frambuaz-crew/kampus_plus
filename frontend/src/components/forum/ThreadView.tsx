...
const PostCard: React.FC<{ post: ForumTopic | ForumReply; isThread?: boolean }> = ({ post, isThread = false }) => {
    ...
    <div className={`mb-6 overflow-hidden rounded-2xl border border-gray-200 bg-white shadow-sm ${isThread ? 'border-l-4 border-l-indigo-600' : ''}`}>
      ...
      <div className="p-8">
        {'title' in post && post.title && (
          <h2 className="mb-6 text-3xl font-black leading-tight tracking-tight text-gray-900">{post.title}</h2>
        )}
        <div className="whitespace-pre-wrap text-base font-medium leading-relaxed text-gray-700">{post.content}</div>
      </div>
      ...
      <div className="space-y-6">
        ...
        {replies.length === 0 && !showReplyForm && (
          <div className="flex flex-col items-center justify-center rounded-3xl border-2 border-dashed border-gray-100 bg-white py-16 text-center shadow-sm">
            ...
            <p className="text-lg font-bold tracking-tight text-gray-400">Henüz kimse bir şey söylememiş.</p>
