...
export const SchedulePDFUploadModal: React.FC<SchedulePDFUploadModalProps> = ({
  defaultUniversity = '',
  defaultClassYear = '',
  defaultSemester = 'Güz',
  defaultAcademicYear = '',
          ...
          {isSuccess && (
            <div className="flex items-center gap-3 bg-green-50 border border-green-200 rounded-xl px-4 py-3 text-sm text-green-700 font-medium">
              ...
              <div>
                ...
                {result && (
                  <p className="text-green-600 text-xs mt-0.5">{result.total_lessons} ders, {result.days_parsed.length} gün eklendi</p>
                )}
              </div>
            </div>
          )}
