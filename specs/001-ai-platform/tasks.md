...
- [x] T078 [US3] Implement `/documents` POST endpoint in `backend/src/api/routes/documents.py`: accept multipart/form-data, validate file type (PDF) and size (<25MB), scan for malware (see T078.5), upload to S3 with user_id prefix, create UserDocument record with `processing_status=pending` ✅ 2025-12-23 (GREEN: Full validation chain - file type/size/malware/quota/S3/DB | Error handling: 400/403/413/500 | ACL enforcement | Background task queued | Commit: 8c7a468)
- [x] T078.5 [US3] **[SECURITY]** Integrate malware scanning for uploaded PDFs: add ClamAV Docker container to `docker-compose.yml`, create `backend/src/services/malware_service.py` with ClamAV client, reject infected files before S3 upload with clear error message ✅ 2025-12-23 (GREEN: ClamAV synchronous scanning integrated in T078 upload endpoint | Rejects infected files with HTTP 400 before S3 upload | Commit: 8c7a468)
- [x] T079 [US3] Implement `/documents` GET endpoint: list user's uploaded documents with metadata (name, size, upload date, processing status) ✅ 2025-12-23 (GREEN: Pagination with skip/limit | Status filtering | Excludes soft-deleted documents | ACL: user_id ownership | Commit: 8c7a468)
- [x] T080 [US3] Implement `/documents/{id}` GET endpoint: retrieve document metadata ✅ 2025-12-23 (GREEN: Full metadata retrieval | ACL: ownership check → 403 if unauthorized | Returns page_count, chunk_count, processing details | Commit: 8c7a468)
- [x] T081 [US3] Implement `/documents/{id}/download` GET endpoint: generate pre-signed S3 URL (15-minute expiry), return URL for client download ✅ 2025-12-23 (GREEN: Pre-signed S3 URL generation | 15-minute expiry | ACL: ownership check | Returns url, filename, expires_in | Commit: 8c7a468)
- [x] T082 [US3] Implement `/documents/{id}` DELETE endpoint: soft-delete UserDocument, remove from S3, delete associated VectorEmbeddings ✅ 2025-12-23 (GREEN: Soft delete with is_deleted=True | S3 lifecycle policy for cleanup | ACL: ownership check | 204 No Content response | Commit: 8c7a468)
- [x] T083 [US3] Create background task in `pdf_service.py`: process_document(doc_id) → download from S3 → extract text → chunk → generate embeddings → add to VDB_Social → update status (PENDING → PROCESSING → COMPLETED on success OR → FAILED on error). Log errors with structured JSON (document_id, error_type, message, stack_trace) ✅ 2025-12-23 (GREEN: Complete PDF processing pipeline - S3 download → PyPDF2 extraction → 512-token chunks → Gemini embeddings → VDB_User storage | Updates processing_status, page_count, chunk_count | Error handling with status=failed | Commit: 6d61e7a)
- [x] T083b [US3] **[RESILIENCE]** Implement retry logic for failed PDF processing: 3 automatic retry attempts with exponential backoff (1min, 5min, 15min). If all retries fail, set `processing_status=failed` with `error_message` field. Add 'Retry Processing' button in DocumentList UI for manual reprocessing of failed documents ✅ 2025-12-23 (GREEN: process_document_with_retry() - Exponential backoff: 1min/5min/15min delays | Final status=failed after all retries exhausted | enqueue_document_processing() integration | Commit: 6d61e7a)
- [x] T084 [US3] Integrate processing task with upload endpoint: trigger async processing after S3 upload (using asyncio background task) ✅ 2025-12-23 (GREEN: asyncio.create_task integration | enqueue_document_processing() called from T078 | with_retry=True for T083b | Commit: 6d61e7a)
